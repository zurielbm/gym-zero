...
for(const [id,name] of [['pec','Pec Fly'],['rear','Rear Delt Fly']]){
 ...
 if(version==='after') {await page.locator('.muscle-map').first().scrollIntoViewIfNeeded();await page.screenshot({path:`artifacts/machine-muscle-report/assets/after-${id}-map.png`});}
}
...
await page.screenshot({path:`artifacts/machine-muscle-report/assets/${version}-workout.png`});await browser.close();
